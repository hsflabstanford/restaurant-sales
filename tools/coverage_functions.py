...
def calculate_coverage(df, 
    ...
    promo_datetime = promos['intro_date'] # Identify introduction date
    ...
    for qualifier, (beginning, end) in periods_to_use.items():
        ...
        # Subset to the data within the intervention period
...
def plot_time_gaps(time_differences, colorbar_max):

    fig, ax = plt.subplots(figsize=(10, 5))

    # Assuming time_differences is a valid 2D array and using pandas DataFrame
    dat = time_differences.T.iloc[:,1:]  # Converting to DataFrame if not already one
...
def plot_time_series_plant_based(loc_id, df, promos, max_ylim=0, freq='D', subset=True):
