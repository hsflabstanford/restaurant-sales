...
restaurants_by_coverage <- read.csv(here("data", "2_palate_data_parquet_cleaned/restaurants_by_4m_coverage.csv")) %>%
...
for (loc_id in restaurants_by_coverage[1:6]) {
  ...
  # Read parameters
  param_grid_entire <- readRDS(here("validation_results", paste0("param_grid_lags_", loc_id, ".rds")))
  param_grid_intervention <- readRDS(here("validation_results", paste0("param_grid_lags_", loc_id, ".rds")))
  ...
  for (outcome in outcomes) {
    ...
    for (data_type in data_types) {
      ...
      for (lag_combo in names(lag_options[[outcome]][[data_type]])) {
        ...
        model_path <- here("modeling_results", "grid_search",
                           paste0(loc, "_", data_type, "_", outcome, "_model.rds"))
        
        diag_path <- here("modeling_results", "grid_search",
                          paste0(loc, "_", data_type, "_", outcome, "_diagplot.png"))
        
        pred_path <- here("modeling_results", "grid_search",
