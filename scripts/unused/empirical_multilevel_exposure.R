...
init_fn <- function(chain_id = 1) {
  ...
  if (K_exposure > 0) {
    init_list$mu_gamma <- rnorm(1, 0, 0.1)
    init_list$sigma_gamma_between <- abs(rnorm(1, 0, 0.5)) + 0.1
    init_list$sigma_gamma_within <- abs(rnorm(1, 0, 0.5)) + 0.1
    init_list$z_eta <- rnorm(R, 0, 1)
