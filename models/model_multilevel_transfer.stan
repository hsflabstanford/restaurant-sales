...
model {
  ...
  mu_beta_random ~ laplace(0, mu_beta_scale);
  mu_beta_fixed ~ laplace(0, mu_beta_scale);
  
  // Exposures
  mu_gamma ~ normal(0, mu_gamma_scale);                     // No shrinkage for the exposures
  
  // INGARCH params
  mu_alpha_random_raw ~ laplace(0, mu_alpha_scale);
  mu_alpha_fixed_raw ~ laplace(0, mu_alpha_scale);
  mu_delta_random_raw ~ laplace(0, mu_delta_scale);
  mu_delta_fixed_raw ~ laplace(0, mu_delta_scale);
